...
function renderCacheList(list,div,kill) {
    ...
    for (var i=0; i<list.length; i++) {
        var le = list[i];
        var row = [new Date(le.startTime).toString('yy/MM/dd HH:mm:ss'), "<a onclick='QM.queryHostsRescan(\""+le.uuid+"\",\""+le.job+"\")' href='#'>"+le.uuid+"</a>", le.alias || '', limit(le.job,15), limit(le.paths[0],80), limit(le.ops,40), fcsnum(le.runTime), fcsnum(le.lines), '<a href="#" onclick="QM.'+kill+'(\''+le.uuid+'\')">x</a>'];
        html += '<tr><td>'+row.join('</td><td>')+'</td></tr>';
    }
    html += '</table>';
    $(div).innerHTML = html;
