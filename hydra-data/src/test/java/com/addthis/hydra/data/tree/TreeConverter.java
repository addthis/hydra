...
public class TreeConverter {
    ...
    public static void main(String args[]) throws Exception {
        ...
        new TreeConverter(new File(args[0]), new File(args[1]), maxNodes);
    }

    public TreeConverter(File readRoot, File writeRoot, int maxNodes) throws Exception {
        long mark = System.currentTimeMillis();
        ReadTree readTree = new ReadTree(readRoot);
        Tree writeTree = new Tree(writeRoot, false, true);
        long openTime = System.currentTimeMillis() - mark;
        long testTime = 0;
        try {
            mark += openTime;
            ReadTreeNode readNode = readTree.getRootNode();
            TreeNode writeNode = writeTree.getRootNode();
    ...
    private void generateNode(Tree writeTree,
            TreeNode writeParent,
            ReadTreeNode readChild) {
        TreeNode writeNode = (TreeNode) writeTree.getOrCreateNode(writeParent, readChild.getName(), null);
        writeNode.writeLock();
    ...
    private void explore(Tree writeTree,
            TreeNode writeNode,
        ...
        if (iter != null) {
            ...
            try {
                while (iter.hasNext()) {
                    ReadTreeNode readChild = (ReadTreeNode) iter.next();
                    TreeNode writeChild = writeTree.getNode(writeNode, readChild.getName(), false);
